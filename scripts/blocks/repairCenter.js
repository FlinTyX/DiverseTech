const spawn = new Effect(52, e => {
    ...
    Draw.rect(Core.atlas.find("diversetech-repairCenter-blend"), e.x, e.y);
    Draw.blend();
    Draw.reset();
});
...
const repairOrb = extend(BasicBulletType, {
    ...
    drawHeal(b, unit, mag, color){
        ...
        Drawf.laser(
            b.team,
            Core.atlas.find("laser"),
            Core.atlas.find("laser-end"),
            b.x - 1 + rand1/2,
            b.y - 1 + rand2/2,
            x, y, 0.35 + rad/12
        );
    ...
    draw(b){
        ...
        Draw.rect(Core.atlas.find("large-bomb-back"), b.x, b.y, pulse, pulse, b.rotation() + (Time.time * 2) + pulse/2);
        Draw.z(Layer.flyingUnit + 1.4);
        Draw.color(Pal.bulletYellow);
        Draw.rect(Core.atlas.find("large-bomb"), b.x, b.y, pulse + 0.5, pulse + 0.5, b.rotation() - (Time.time * 2));
